...
def render_animation(skeleton, poses_generator, algos, t_hist, fix_0=True, azim=0.0, output=None, size=6, ncol=5, bitrate=3000):
    ...
    animating = True
    find = 0
    hist_lcol, hist_rcol = '#e66f51', '#42a6cb'
    pred_lcol, pred_rcol = '#a64036', '#4182a4'
    ...
    def update_video(i):
        ...
        if not initialized:
            ...
            for j, j_parent in enumerate(parents):
                ...
                zo = 1 if col == rcol else 2
                # zo = 2 if j in skeleton.joints_right() else 1
                for n, ax in enumerate(ax_3d):
                    pos = poses[n][i]
                    lines_3d[n].append(ax.plot([pos[j, 0], pos[j_parent, 0]],
                                               [pos[j, 1], pos[j_parent, 1]],
                                               [pos[j, 2], pos[j_parent, 2]], zdir='z', c=col, zorder=zo, lw=6))
            initialized = True
        else:
            ...
            for j, j_parent in enumerate(parents):
                ...
                for n, ax in enumerate(ax_3d):
                    ...
                    pos = poses[n][i]
                    # pos = np.array(pos)
                    lines_3d[n][j - 1][0].set_xdata(np.asarray([pos[j, 0], pos[j_parent, 0]]))
                    lines_3d[n][j - 1][0].set_ydata(np.asarray([pos[j, 1], pos[j_parent, 1]]))
                    lines_3d[n][j - 1][0].set_3d_properties(np.asarray([pos[j, 2], pos[j_parent, 2]]), zdir='z')
